 ...
 * expiresIn - The time in milliseconds until the file expires
 * maxSize - The maximum folder size in bytes until the retention guard starts to delete expired files
 * minFreeSpace - The minimum volume free space size in bytes until the retention guard starts to delete expired files
 * mtime - If to test file expiration on the modification time property (Used by default)
 * ctime - If to test file expiration on the creation time property (Used by default)
 * atime - If to test file expiration on the access time property (Not used by default)
 * @constructor
 */
internals.RetentionGuard = function(dir, options){
    ...
    // By default - true
    this.mtime = !(options.mtime === false);
    this.ctime = !(options.ctime === false);

    // By default - false
    this.atime = (options.atime === true);
};
